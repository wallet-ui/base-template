...
export function getWalletErrorMessage(err: unknown, fallbackMessage: React.ReactNode): React.ReactNode {
    if (isWalletStandardError(err, WALLET_STANDARD_ERROR__FEATURES__WALLET_ACCOUNT_FEATURE_UNIMPLEMENTED)) {
        return (
            <>
                This account does not support the <code className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-sm">{err.context.featureName}</code> feature
            </>
        );
        ...
        return (
            <div className="flex flex-col gap-4">
                <p>
                    ...
                    ) does not support the <code className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-sm">{err.context.featureName}</code> feature.
                </p>
                <div>
                    Features supported:
        ...
        return (
            <div className="flex flex-col gap-4">
                <p>
                    This account does not support the chain <code className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-sm">{err.context.chain}</code>.
                </p>
                <div>
                    Chains supported:
                    <ul className="list-disc pl-5 mt-2">
